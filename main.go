...
func main() {
	...
	if !(*isFirst) {
		...
		// MODIFICATION MINI-PROJET 3
		otherPeers.peers = handleArrival(conn, *port)
	...
	for _, peer := range otherPeers.peers {
		go peer.receiveMessages(receiveChan)
	...
	go otherPeers.prepareChatMessages()
	go displayChatMessages(receiveChan)
	...
	for {
		...
		ip, ok := handleConnection(conn, &otherPeers)
		if ok {
			// MODIFICATION MINI-PROJET 3
			...
			go newPeer.receiveMessages(receiveChan)
			go newPeer.sendMessages()
		}
...
func handleConnection(conn net.Conn, otherPeers *peerCollection) (string, bool) {
...
func handleArrival(conn net.Conn, myPort int) []peer {
